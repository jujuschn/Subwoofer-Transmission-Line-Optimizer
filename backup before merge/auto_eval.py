...
class ABEC:
    ...
    # TODO    
    def load(self, pipe) :
        # access LEScript-file and write in the translation of the pipe in script language 
        return
    ...
    def return_data(self) :
        ...
        self.df = pd.read_csv(StringIO(data_str), delim_whitespace=True, header=None, names=['frequency','spl','phase'])
        print(self.df.head())
    """
    def return_data(self): #gpt version "more rigorous"
        # Read the file safely
        with codecs.open(self.output_file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        ...
        try:
            ...
            end_index = next(i for i, line in enumerate(lines) if line.strip() == end_marker)
        except StopIteration:
            raise ValueError("The data section delimiters 'Data' and 'Data_End' were not found.")

        # Extract data lines, strip and filter empty lines
        data_lines = [line.strip() for line in lines[start_index:end_index] if line.strip()]
        
        # Optional: Validate line format (should have 3 columns)
        valid_lines = []
        for line in data_lines:
            parts = line.split()
            if len(parts) != 3:
                raise ValueError("Invalid data line (expected 3 columns): %s" % line)
        ...
        # Parse using pandas - space-separated, no header
        self.df = pd.read_csv(StringIO(data_str), 
                            delim_whitespace=True, 
                            header=None, 
                            names=["frequency", "spl", "phase"])

        # Optional: Type checking and conversion (scientific safety)
